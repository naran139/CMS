...
def staff_home(request):
    ...
    context = {
        ...
        'total_unchecked_assignments': total_unchecked_assignments,
        'subjects':subjects,
...
def give_assignment(request):
    ...
    # Filter subjects taught by the current staff member
    subjects = Subject.objects.filter(staff=current_staff)
    if not subjects:
        # If no subjects found, display a message to the user
        messages.error(request, "You don't have any subjects assigned. Please contact your administrator.")
        return redirect(reverse('staff_home') ) # Redirect to the homepage or any other appropriate page
    ...
    if request.method =="POST":
        ...
        subject_id = request.POST.get('subject_id')  # Assuming you pass subject_id in the form
        subject = get_object_or_404(Subject, pk=subject_id)
        staff = request.user.staff  # Assuming staff is related to the user
        course = subject.course

        # Retrieve all students enrolled in the course associated with the subject
        ...
        assignment.save()
        messages.success(request, "Assignment Uploaded Successfully.")  # Pass the appropriate message here
        return redirect('give_assignment')
    return render(request,"staff_template/give_assignment.html",context)
...
def manage_assignment(request):
    ...
    # Get all assignment submissions associated with the staff member's subjects
    submitted_assignments = AssignmentSubmission.objects.filter(assignment__subject__staff=staff)

    context = {
        'page_title': "View Submitted Assignments",
...
def remove_submission(request):
        submission_id = request.POST.get('submission_id')
        try:
        # Assuming you have a model called Submission and a method to delete submissions
            submission = AssignmentSubmission.objects.get(id=submission_id)
            submission.delete()
            return JsonResponse({'message': 'Submitted Assignment removed successfully'}, status=200)
        except AssignmentSubmission.DoesNotExist:
           return JsonResponse({'error': 'Assignment submission not found.'}, status=404)
        except Exception as e:
            return JsonResponse({'error': str(e)}, status=500)
...
def submitted_assignments(request):
    # Calculate the cutoff date (e.g., 1 minute ago)
    cutoff_date = timezone.now() - timedelta(minutes=1)
    
    # Query for new assignment submissions created within the last minute
    new_assignments = AssignmentSubmission.objects.filter(submission_date__gte=cutoff_date)
    
    # Return the count of new assignment submissions as a JSON response
